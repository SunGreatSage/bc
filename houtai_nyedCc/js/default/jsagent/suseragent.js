...
function myready() {
	...
	$("#optionselect").click(function() {
		if ($("#optionselect").val()=="选择项▼"){
		 $('.option').show();
		 $('#optionselect').val("选择项▲");
				} else {
		 $('.option').hide();
		 $('#optionselect').val("选择项▼");
		 }
	...
	$(".back").click(function() {
		$("#drownMenu li:last",parent.document).remove();
		var fid = $("#fid").val();
		$.ajax({
			type: 'POST',
			url: mulu + 'suser.php',
  ...
  $(".zcmxtb input.copysend").click(function(){
		...
		$.ajax({
			...
			$(".account a.maxmoney").click(function(){
				...
				$(".edudiv .ui-dialog-title").html("" + username + "#修改额度");
			...
			$(".user_tb a.record").click(function() {
				...
				$.ajax({
					...
					success: function(m) {
						...
						var estr = "<thead><tr><th>操作是</th><th>操作者</th><th>IP</th><th>地址</th><th>操作内容</th></tr></thead><tbody>";
						for (i = 0; i < el; i++) {
							estr += "<tr>";
...
function editpoints(uid) {
	$.ajax({
		...
		success: function(m) {
			...
			$("#limitSettingList").append(loadmask);
			$("#limitSettingList").addClass('masked-relative masked');
		},
		complete:function(){
			$(".loadmask").remove();
            $("#limitSettingList").removeClass('masked-relative masked');			
        },
					success: function(m) {
						if (Number(m) == 1) {
							alert("退水设置已成功保存。");
