...
function myready() {
   ...
   if (/MSIE (6|7)/.test(navigator.userAgent)) {
    ...
    $(".header .top_menu  a").click(function () {
        ...
        var a = $(this);
        a = a.html();
        if ("个人管理" == a || "自动补货" == a || "用户管理" == a || "报表查询" == a || "即时注单" == a || "开奖结果" == a || "规则说明" == a || "注单预警" == a ) {
           var i = $(this).index();
    ...
    $(".top_menu  a").click(function () {
        ...
        if (url == 'suser') {
            ...
            frame.window.location.href = mulu+"libset.php?xtype=warn"
        } else if (url == 'admins') {
...
function kj() {
    ...
    var str = '';
    for (i = 0; i < ul; i++) {
        if (ngid == 100 & i == 6) str += "<label class='plus'>+</label>";
        str += qiu(upkj[i])
		//console.log(upkj);
    }
	$(".openResult").html(str);  
...
function qiu(n, bname) {
    ...
    return "<span class='ball_" + n + "'></span><em>鼠</em>";
}
