...
function myready() {
	...
	$(".baotit",parent.document).html("公司报表查询");
	$(".bbzl").html("代理报表种类");
	...
	$.ajax({
		...
		success: function(m) {
			...
			for (i = 0; i < ml; i++) {
				...
				str += "<td  class='name' align='center' ><a href='javascript:void(0)' class='mx' str='" + mxstr + "'>明细</a></td>";
				str += "</tr></tbody>";
...
function agentsearch() {
	...
	$.ajax({
		...
		success: function(m) {
			...
			if (qishu == '') {
				if (end != start) {
					$(".baotit",parent.document).html( "（" + nowusername + "） - 交收报表 [" + start + " — " + end + "] ")
				} else {
					$(".baotit",parent.document).html( "（" + nowusername + "） - 交收报表 [" + start + "] ")
				}
			} else {
				$(".baotit",parent.document).html( "（" + nowusername + "） - 交收报表 [" + $("input[name='lottery']:checked").attr('gname') + " — " + qishu + "期] ")
			}
			m = m['bao'];
			var ml = m.length;
			var str = '';
...
function agentfl() {
	...
	$.ajax({
		...
		success: function(m) {
			...
			if (qishu == '') {
				if (start != end) {
					$(".baotit",parent.document).html(nowlayername + "（" + nowusername + "） - 分类报表 [" + start + " — " + end + "] ")
				} else {
					$(".baotit",parent.document).html(nowlayername + "（" + nowusername + "） - 分类报表 [" + start + "] ")
				}
			} else {
				$(".baotit",parent.document).html(nowlayername + "（" + nowusername + "） - 分类报表 [" + $("input[name='lottery']:checked").attr('gname') + " — " + qishu + "期] ")
			}
...
function agentnew() {
	...
	$.ajax({
		...
		success: function(m) {
			...
			if (qishu == '') {
				if (start != end) {
					$(".baotit",parent.document).html(nowlayername + "（" + nowusername + "） - 交收报表 [" + start + " — " + end + "] ")
				} else {
					$(".baotit",parent.document).html(nowlayername+ "（" + nowusername + "） - 交收报表 [" + start + "] ")
				}
			} else {
				$(".baotit",parent.document).html(nowlayername + "（" + nowusername + "） - 交收报表 [" + $("input[name='lottery']:checked").attr('gname') + " — " + qishu + "期] ")
			}
