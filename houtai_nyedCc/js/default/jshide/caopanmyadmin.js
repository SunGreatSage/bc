...
function myready() {
	...
	$(".add").click(function() {
		...
		$("#addbtn").val("新增");
		$("div.addtb .ui-dialog-title").html("新增管理员")
	});
	$(".edit").click(function() {
...
function showrecord(aid, username) {
	$.ajax({
		...
		success: function(m) {
			...
			var estr = "<thead><tr><th>操作时间</th><th>充值者</th><th>IP</th><th>状态</th></tr></thead>";
			for (i = 0; i < el; i++) {
				estr += "<tr>";
...
function showlogininfo(aid, username) {
	$.ajax({
		...
		success: function(m) {
			...
			var lstr = "<thead><tr><th>登陆时间</th><th>IP</th><th>地址</th><th>状态</th></tr></thead>";
			for (i = 0; i < ll; i++) {
				lstr += "<tr>";
