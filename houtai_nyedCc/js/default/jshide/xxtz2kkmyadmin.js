function myready() {
		/*$.ajaxSetup({
        beforeSend:function (){
			var loadmask = '<div class="loadmask"></div><div class="loadmask-msg" style="top: 30%;left: 50%;"><div style="">正在处理......</div></div>';
			$(".user").append(loadmask);
			$(".user").addClass('masked-relative masked');
		},
		complete:function(){
			$(".loadmask").remove();
            $(".loadmask-msg").remove();	
            $(".user").removeClass('masked-relative masked');			
        },
      });*/
	...
	$("#zkgjmx").click(function() {
			if ($("#zkgjmx").val()=="展开▼"){
		 $('.gzmx').show();
		 $('.nowtb').addClass('wh2500');
		 $('#zkgjmx').val("关闭▲");
				} else {
		 $('.gzmx').hide();
		  $('.nowtb').removeClass('wh2500');
		 $('#zkgjmx').val("展开▼");
		 }
    });
	...
	/*if (layer < 8) {
		$(".nowtb tr:eq(0)").append("<th>所属" + layername[layer] + "</th>")
	}*/
	$(".nowtb tr:eq(0)").append("<th><strong>公司金额</strong></th>");
	for (i = 0; i < clayer - 1; i++) {
		$(".nowtb tr:eq(0)").append("<th colspan=3  class='gzmx' style='display: none;'><strong>" + layername[i] + "/占/赔/水</strong></th>")
	}
	var str = '';
	/*for (i = 0; i < clayer - 1; i++) {
		str += "<th>占</th><th>赔</th><th>退</th>"
	}
	if (str != '') $(".nowtb2").append("<tr class='bt'>" + str + "</tr>");*/
	$("label").addClass('red');
				...
				if ($(this).attr('layer') == '1') {
					for (k = 0; k < layernames.length; k++) {
						if (Number($(this).attr('wid')) == layernames[k]['wid']) {
							...
							$(".nowtb tr:eq(0)").append("<th rowspan=2>公司</th>");
...
function getxx(uid) {
	...
	$.ajax({
		...
		//	console.log(m['sql']);
			...
			for (i = 0; i < ml; i++) {
				str += "<tr>";
				str += "<td><input type='checkbox' id='" + m['tz'][i]['id'] + "' gid='" + m['tz'][i]['gids'] + "'  z='" + m['tz'][i]['z'] + "'  value='"+ m['tz'][i]['tid'] + m['tz'][i]['userid'] + "' /></td>";
				str += "<td><a href='javascript:void(0);'  class='edit2'>修改</a>  ";
                if((m['tz'][i]['z']==7)){
				str += "<a href='javascript:void(0);' style='color: red;' class='edit'>恢复</a>";
				}else{
				str += "<a href='javascript:void(0);' class='edit'>注销</a>";
				}				
				str += " <a href='javascript:void(0);' class='del'>删除</a></td>";
				...
				if((m['tz'][i]['fl']==107 & m['tz'][i]['bids']!=23378805 ) | (m['tz'][i]['fl']==101 & m['tz'][i]['bids']==23378755) ){
				    ...
				    str += "<td style='color: blue;'>" + m['tz'][i]['cid'] + "-" + inputs('pid', m['tz'][i]['pid']) + "</td>";
				}
				...
				str += "<td style='color: red;'>" + inputs('peilv1', m['tz'][i]['peilv1']) + '/' + m['tz'][i]['peilv2'] + "</td>";
				str += "<td>" + inputs('points', m['tz'][i]['points']) + "</td>";
				str += "<td>" + m['tz'][i]['user'] + "</td>";
				str += "<td>" + inputs('time', m['tz'][i]['time']) + "</td>";
				str += "<td>" + m['tz'][i]['zc' + 0] + "</td>";
				for (j = melayer; j < maxlayer; j++) {
					if (j != 0) {
						str += "<td class='gzmx' style='display: none;' >" + m['tz'][i]['zc' + j] + "</td>";
						str += "<td class='gzmx' style='display: none;'>" + inputs('peilv1' + j, m['tz'][i]['peilv1' + j]) + "</td>";
						str += "<td class='gzmx' style='display: none;'>" + inputs('points' + j, m['tz'][i]['points' + j]) + "</td>"
					}
				}
				str += "</tr>"
			}
			...
			pstr += '</select>'
	       $(".nowtb").append(str);
		   $(".nowtb").append("<tr><td colspan=40>选择页数：" + pstr + "</td></tr>");
...
function selectjs(classes,val){
	...
	varr = [1,0,2,9,7];
	var str="<select class='"+classes+"' style='max-width:70px;' >";
	for( var i in varr){
...
function addfunc() {
	...
	$(".editall2").unbind('click');
		$(".editall2").click(function() {
		var str = '{';
		var i = 0;
		$(".nowtb input:checkbox").each(function() {
			if ($(this).prop('checked') == true & !$(this).hasClass('clickall') & $(this).val() != undefined) {
		...
		$.ajax({
			type: 'POST',
			url: mulu + 'xxtz2.php',
			data: 'xtype=edittz2&str=' + str+"&start="+start+"&end="+end,
			cache: false,
			success: function(m) {
				console.log(m);
				if (Number(m) == 1) {
