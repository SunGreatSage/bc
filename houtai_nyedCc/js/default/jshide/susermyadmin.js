...
/*	$(".userzzh",parent.document).click(function(){
		window.location.href = mulu+"caopan.php?xtype=show";
	})*/
	...
	$.ajax({
		...
		success: function(m) {
			...
			for (var i in m) {
			    ...
			    strd+= "<li type='" + m[i]['layertype'] + "' layer='" + m[i]['layer'] + "' name='" + m[i]['name'] + "'><a href='javascript:void(0)'>" + m[i]['name'] + "&gt;(" + m[i]['num'] + ")</a></li>";
			...
			$(".dropdown-menu-shadow",parent.document).find("li").remove();
			$(".dropdown-menu-shadow",parent.document).append(strd);
			$(".dropdown-menu-shadow li[type='0']",parent.document).hide();
			$(".dropdown-menu-shadow li[type='1']",parent.document).hide();
			$(".dropdown-menu-shadow li[type='3']",parent.document).hide();
			$(".dropdown-menu-shadow li[type='4']",parent.document).hide();
			str = null;
			strd = null;
			...
			$(".dropdown-menu-shadow li",parent.document).click(function() {
				layertype = Number($(this).attr('type'));

				if (layertype == 2) {
					$("#layer").val($(this).attr('layer'));
				}
				$(".title label:eq(1)").html($(this).attr('name'));
				$(".dropdown-menu-sub-indicator",parent.document).removeClass("dropdown-menu-hover");
			    $(".dropdown-menu-shadow",parent.document).hide();
			    $(".dropdown-menu-shadow",parent.document).css("visibility","hidden");
				$(".accessible",parent.document).html("级别："+$(this).attr('name')+"<span class='dropdown-menu-sub-indicator'>»</span>")
				getuser();
			});
			
			var lil = $(".left_panel li").length;
			$("#directAgentCount",parent.document).html("("+$(".left_panel li:eq(1) span").html()+")");
			$("#allAgentCount",parent.document).html("("+$(".left_panel li:eq("+(lil-2)+") span").html()+")");
			$("#directMemberCount",parent.document).html("("+$(".left_panel li:eq(2) span").html()+")");
			$("#allMemberCount",parent.document).html("("+$(".left_panel li:eq("+(lil-1)+") span").html()+")");
			//console.log($(".left_panel li:eq(1)").tel);
...
function getuser() {
	...
	$.ajax({
		...
		//	//changeh(document.documentElement.scrollHeight + 500);
			...
			$(".record span").html($(".pageinfo").attr('rcount'));
			var pcount = Number($(".pageinfo").attr('pcount'));
			$(".page_count span").html(pcount);
			upage = Number($(".pageinfo").attr('upage'));
			var pstr = "<li><a class='current2 prev'>《上一页</a></li>";
			for (i = 1; i <= pcount; i++) {
				if (i == upage) {
					pstr += "<li><a class='current'> " + i + " </a></li>";
				} else {
					pstr += "<li><a href='javascript:void(0)' class='p'>" + i + "</a></li>";
				}
			}
			pstr += "<li><a class='current2 next'>下一页》</a></li>";
			...
			$(".user_tb a.showdown").click(function() {
                ...
                var str = "<li style='font-size: 12px;'><a href='javascript:void(0)'  uid='"+obj.attr('uid')+"'  username='"+obj.attr('username')+"' layertype='"+obj.attr('layertype')+"' lyname='"+obj.attr('lyname')+"' style='display: inline' class='' >"+obj.attr('username')+" ("+obj.attr('lyname')+")</a></li>";
                $("#drownMenu",parent.document).append(str);
				$(".accessible",parent.document).html("选择用户级别<span class='dropdown-menu-sub-indicator'>»</span>")
				gettree();
				getuser();
			});
           	//位置
			$("#drownMenu li a",parent.document).unbind('click');
			$("#drownMenu li a",parent.document).click(function() {
				if(!$(this).hasClass("one")){
				layertype = 0;
				$("#fid").val($(this).attr('uid'));
				$(".title").find("label:eq(0)").html($(this).attr('username'));
			...
			$(".user_tb a.record").click(function() {
				...
				$.ajax({
					...
					success: function(m) {
						...
						var estr = "<thead><tr><th>操作时间</th><th>操作者</th><th>IP</th><th>地址</th><th>操作内容</th></tr></thead><tbody>";
						for (i = 0; i < el; i++) {
							estr += "<tr>";
			...
			$(".user_tb a.logininfo").click(function() {
				...
				$.ajax({
					...
					success: function(m) {
						...
						var lstr = "<thead><tr><th>登陆时间</th><th>IP</th><th>地址</th><th>登陆状态</th></tr></thead><tbody>";
						for (i = 0; i < ll; i++) {
							lstr += "<tr>";
...
function editmoneypass(obj,objs) {
	...
	$(".zcmxtb .moneypasssend").click(function() {
		...
		if(!ptest.test(v1) || !ptest.test(v2) || v1!=v2){
			alert("密码必须由字母数字组成 ，6-20位（不区分大小写）！");
			return false;
		}
		if(v1!=v2){
	...
	$.ajax({
		...
		success: function(m) {
			...
			for (i = 0; i < cg; i++) {
				...
				str = '<div class="game_tab" style="margin: 4px 4px 0 0;height: 30px;" ><a>' + g[i]['gname'] + '</a></div>';
            ...
            $(".utb input.slowbtn").click(function(){
			     var val = Number($(this).prev().val());
				 var pk = $('#selectsetting').val();
				 var v1 = pk != '0' ? ''+pk : '';
				 if(v1=='A') var v1='a';
				 if(v1=='B') var v1='b';
				 if(v1=='C') var v1='c';
				 if(v1=='D') var v1='d';
				 var obj = v1 ? $('[id^="points'+v1+'"]:enabled') : $('[id^="points"]:enabled');
				 obj.each(function(){
				...
				$(".utb input.zpbtn").click(function(){
			     var val = Number($(this).prev().val());
				 //console.log(val);
				 $("table.cs2 td.plcs input:text").each(function(){
			         if($(this).parent().hasClass("plcs")){
				         var v1 = val;
						 var v2 = Number($(this).attr('m'));
						 if(v1<0) v1=0;
						 if(v1>v2) v1=0;
						 $(this).val(getResult(v1,4));
					 }
			...
			$(".utb input.wtzpbtn").click(function(){
			     var val = Number($(this).prev().val());
				 //console.log(val);
				 $("table.cs2 td.plcs input:text").each(function(){
			         if($(this).parent().hasClass("plcs")){
				         var v1 = Number($(this).val())+val;
						 var v2 = Number($(this).attr('m'));
						 if(v1<0) v1=0;
						 if(v1>v2) v1=0;
						 $(this).val(getResult(v1,4));
			...
			$(".setpointssend").click(function() {
				...
				//	async: false,
					data: 'xtype=setpoints' + str,
					 beforeSend:function (){
			 $("#limitSettingList").mask("正在执行...");
		},
		complete:function(){
			$("#limitSettingList").unmask("正在执行...");	
        },
					success: function(m) {
						if (Number(m) == 1) {
							alert("退水设置已成功保存。");
	...
	$.ajax({
		...
		success: function(m) {
			...
			for (i = 0; i < cg; i++) {
				...
				str = '<div class="game_tab" style="margin: 4px 4px 0 0;height: 30px;"><span loto="MCHK6" group="HK6" class="group_tap group_tap_focus">' + g[i]['gname'] + '</span></div>';
				str += '<table class="layout cs2"><tbody><tr>{tbody}</tr></tbody></table>';
				...
				for (j = 0; j < cp; j++) {
					...
					if (plc == 1 & Number(ta['son']) > 1) {
						for (var n in ta['cs']) {
							...
							//data += "<td></td>";
							data += "<th><!--<input type='checkbox' class='son b" + ta['cs'][n]['bc'] + "' /><input type='button' class='cstb' value='同选中' />&nbsp;&nbsp;-->";
							data += "" + ta['cs'][n]['name'] + "</th>";
...
function checkpass() {
	...
	if(!ptest.test(pass1)){
		...
		$("#passdMsg").html("<em class='error'>密码必须由字母数字组成 ，6-20位（不区分大小写）！</em>");
		$("#password").parent().find('img').attr('src', globalpath + 'img/0.gif');
		return false
	}
	//$("#password").parent().find('img').show();
	$("#passdMsg").html("<em class='success'></em>");
...
function deluser(ustr) {
	if (!confirm("确定删除用户吗？")) return false;
	var pass = prompt("请输入密码:", '');
	//console.log(ustr);
	$.ajax({
		type: 'POST',
...
function rclass(name) {
  ...
  var t_BALL = "正碼|";
  var t_LM="|两面|总肖|";
  var t_ITEM = "波色|特肖|一肖|尾数|正肖|七色波";
  var t_MP = "连码|不中|生肖連|尾数連|过关|";
  var t_="半波|特头数|五行|特尾数|合肖|正肖";
