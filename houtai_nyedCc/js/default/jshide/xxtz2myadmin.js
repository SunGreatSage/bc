function myready() {
	...
	$(".nowtb tr:eq(0)").append("<th>公司金额</th>");
	for (i = 0; i < clayer - 1; i++) {
		$(".nowtb tr:eq(0)").append("<th colspan=3  class='gzmx' style='display: none;'>" + layername[i] + "/占/赔/水</th>")
	}
	var str = '';
	/*for (i = 0; i < clayer - 1; i++) {
	...
	$.ajax({
		...
		success: function(m) {
			...
			for (i = 0; i < ml; i++) {
				...
				if((m['tz'][i]['z']==7)){
				str += "<td><a href='javascript:void(0);' style='color: red;' class='edit'>恢复</a>";
				}else{
				str += "<td><a href='javascript:void(0);' class='edit'>注销</a>";
				}
				...
				str += "<td style='color: blue;'>" + m['tz'][i]['cid'] + "-<span class='pid'>" + m['tz'][i]['pid']+"</span></td>";
				...
				str += "<td style='color: #ce0000;'>注单取消</td>";
				}else{
				str += "<td>未结算</td>";
				}
				str += "<td>" + m['tz'][i]['con'] + "</td>";
				str += "<td>" + m['tz'][i]['je'] + "</td>";
				str += "<td>@<span style='color: red;'>" + m['tz'][i]['peilv1'] +'/'+ m['tz'][i]['peilv2'] + "</span></td>";
				...
				for (j = melayer; j < maxlayer; j++) {
					if (j != 0) {
						...
						str += "<td class='gzmx' style='display: none;'>@<span style='color: red;'>" + m['tz'][i]['peilv1' + j] + "</span></td>";
						str += "<td class='gzmx' style='display: none;'>" + m['tz'][i]['points' + j] + "</td>"
					}
				}
				str += "</tr>"
			}
...
function addfunc() {
    ...
    str += '"' + i + '":{"qishu":"' + obj.find("td:eq(3)").html() + '","gid":"' + $(this).attr("gid") + '","tid":"' + $(this).val() + '","pid":"' + obj.find(".pid").html() + '"';
