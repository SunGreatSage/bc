var gatt, time0, time1, settime0, settime1, timek, settimek, gntime = 30,setgntime, upl, fastobj,tu,gnews;
...
//	$("div.control input:text:eq(0)").focus();
	...
	$(".sub div a:eq("+menu+")",parent.document).addClass('c_curr');
	$("a.ab:eq(0)").addClass('on');
...
/*$(".news button.close",parent.document).click(function(){$(".news",parent.document).hide();});*/
	...
	$(".sub div li",parent.document).click(function() {
       ...
       if(!$("div.control input:checkbox").prop("checked")){
		...
		$(".sub div a",parent.document).removeClass('c_curr');
		$(".sub div a:eq("+index+")",parent.document).addClass('c_curr');
		$("#gameName").html($(".main a.click").html());
		if (fenlei == 100) {
			var bname=$(".main a.click").html();
			if (bname == '特碼' | bname == '正特' | bname == '正碼') {
				$("a.ab").show();
			    $("a.ab:eq(0)").html(bname + "A");
		        $("a.ab:eq(1)").html(bname + "B");
		...
		success: function(m) {
		   ...
		   //alert(m[0]);
			...
			if (m[0] != 'A') {
			  ...
			  if(m[4][0]!='' & m[4][0]!=null){	
				obj.attr('v',m[5]);
				obj.find("div:eq(0)").html("<span>"+ m[6]+"</span>");
				...
				for(i=0;i<ml;i++){
				   ...
				   if(fenlei==163){
				     if(m[4][i]!='' ){
                        ...
                        str += "<span><b class='b"+m[4][i]+" a'>"+m[4][i]+"</b><i>"+m[10][i]+"</i></span>";
				   }else{
					 if(m[4][i]!='' & m[4][i]!=null){
						 if(fenlei==101){
	...
	$.ajax({
		...
		success: function(m) {
			...
			for (i = 0; i < ml; i++) {
				...
				str += "<li><p style='white-space:nowrap;overflow: hidden; text-overflow:ellipsis;'>单号：：<span class='bid'>"+m[i]['tid']+"</span></p><p class='contents'><span class='text' style='color:blue;'>"+m[i]['name']+"</span>&nbsp;@&nbsp;<span style='color:red;'>"+peilv+"</span></p><p>下注额：￥"+m[i]['je']+"</p></li>";
			}
			$(".noBet",parent.document).hide();	
			obj.html(str);
		}
	})
}
...
function getatt() {
	...
	var abcd = $(".p_curr", parent.document).text();
...
function setAmount(n) {
  var currentValue = parseInt($(".zje").val()) || 0;
  var newValue = currentValue + n;
  $(".zje").val(newValue);
  $("td.selected").find("input[type=text]").val(newValue);
  $(".lists td.select").find("input:text").val(newValue);
  $(".ftdiv.selected").find("input:text").val(newValue);
}
