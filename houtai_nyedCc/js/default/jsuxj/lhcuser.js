function exe() {
		...
		if (mode == 1) {
			...
			if (bname == '連碼' | bname == '不中') {
			   aone.sort(function(x,y){return x-y;});
			    $("#chooseItem",parent.document).show();
			    $("#chooseItemhm",parent.document).html(aone.join(","));
			    $("#chooseItemnum",parent.document).html(yl);
			    $(".betListjh",parent.document).html("组合清单");
	...
	for (j = 0; j < i; j++) {
		...
		if (bname == '合肖' | bname == '連碼' | bname == '不中' | bname == '生肖連' | bname == '尾數連') {
			str += "<td>" + play[j]['name'] + '--<label>' + play[j]['con'].join('-') + "</label></td>"
		} else if (bname == '過關' | bname == '过关') {
			str += "<td>" + play[j]['classx'] + '--<label>' + play[j]['con'] + "</label></td>"
		} else {
			str += "<td>" + play[j]['classx'] + '--<label>' + play[j]['name'] + "</label></td>"
		}
		str += "<td  class='odds'>" + play[j]['peilv1'];
		if (play[j]['name'] == '三中二' | play[j]['name'] == '二中特') {
			str += '/' + play[j]['peilv2']
		}
		str += "</td>";
		str += "<td class='amount'><input style='width:60px;' value='" + play[j]['je'] + "' /></td>";
...
function addfunc2() {
	...
	if(!err){
		...
		$("#chooseItem",parent.document).hide();
		$(".betListjh",parent.document).html("项目");
		//obj.find(".cgts").show();
		...
		success: function(m) {
			...
			if(bname == '番摊' ){
				...
				for (i = 0; i < ml; i++) {
					...
					str += " /></td>";
					str += "</tr>"
					//console.log(img(m[i]['name'], bname));
				}
				$(".make").html(str);
			}else if (bname == '合肖' | bname == '連碼' | bname == '不中' | bname == '生肖連' | bname == '尾數連') {
				var tmp = '';
				...
				var tmp = '';
				var str = "";
				str += "<thead><tr class='head'>";
				//str += "<th>项目</th>";
				for (i = 0; i < ml; i += 15) {
					str += "<th colspan=3 class='bt' >" + m[i]['sname'] + "</th>"
				}
				str += "</tr><tr>";
				for (i = 0; i < ml; i += 15) {
					str += "<th nowrap='nowrap'>号码</th><th nowrap='nowrap'>赔率</th><th nowrap='nowrap'>金额</th>";
				}
				str += "</tr><thead>";
	...
	var abcd = $(".p_curr", parent.document).text();
		...
		$.ajax({
			...
			success: function(m) {
				...
				/*	var hm =$(".duotb .name.names.selected span").html();
					console.log(hm);*/
					//$(".chosehm").html(hm);
					if (mode == 2) {
				...
				if ($(this).hasClass('selected')) {
					$(".make input:text").each(function(i) {
						...
						if (i <= 48 & $.inArray(pname, tmp) != -1) {
							...
							$(".make td.p" + pid).removeClass("selected");
							$(".make th.m" + pid).removeClass("selected");
							$(".fasttb td:eq("+i+")").removeClass("selected");
						}
					...
					$(".make input:text").each(function(i) {
						...
						if (i <= 48 & $.inArray(pname, tmp) != -1) {
							...
							$(".make td.p" + pid).addClass("selected");
							$(".make th.m" + pid).addClass("selected");
							$(".fasttb td:eq("+i+")").addClass("selected");
							console.log(i);
							
						}
						/*if (i <= 48 & $.inArray(pname, tmp) == 1) {
						$(".fasttb td." + i).addClass("selected");
						}*/
						//console.log(i);

					});
					if(!$(this).hasClass("xzbtn")){
					$(this).addClass('selected');
					} 
				}
			}
