...
if($msql->f(0)=='x_lib_total'){
    ...
    $bigdata=1;
        ...
        for ($i = 0; $i < count($ustr); $i++) {
          ...
          /*  if ($fsql->f('id') != '') {
			...
			//	$msql->query("delete from `$tb_lib` where userid in (select userid from `$tb_user` where fid{$msql->f('layer')}='{$msql->f('userid')}' )");
    ...
    case "resetpoints":
		...
		//var_dump("insert into `$tb_points` select NULL,gid,$uid,class,ab,a,b,c,d,cmaxje,maxje,minje  from   `$tb_points`  where userid='$fid'");
        userchange("恢复退水", $uid);
        echo 1;
        break;
    case "resetpl":
