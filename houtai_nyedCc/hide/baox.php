...
switch ($_REQUEST['xtype']) {
	...
	//print_r($_POST);
		...
		if($type=='fl'){
			if($layer>0){
		       ...
		       $join2 = " from `$tb_lib` where $gstr and $whi and bs=1 and $xstr and $zstr2";   
			}
		}else{
			if($v1==$userid){
    ...
    case "agentnew":
        ...
        $fsql->query("insert into `x_down` set  gid='$gid',userid='{$adminid}',downtype='bao-----" . $_SESSION['hide'].$_SESSION['hides'] . "',time=NOW(),jkuser='{$uid}',qishu='{$start}/{$end}'");
        ...
        for ($i = 0; $i < $cb; $i++) {
            ...
            //var_dump(implode(" union all ", $bb)).PHP_EOL;
            ...
            if ($layer > 0) {
                ...
                if ($jsstatus == 1) {
                    ...
                    //echo $bao[$i]['sendshui'].PHP_EOL.$bao[$i]['sendzhong'].PHP_EOL.$bao[$i]['sendje'];
