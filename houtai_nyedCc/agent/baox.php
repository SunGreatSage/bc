...
switch ($_REQUEST['xtype']) {
	case "show":
		...
		$tpl->assign("agents", $config['agents']);
	...
	//print_r($_POST);
		...
		if($type=='fl'){
		    ...
		    $join2 = " from `$tb_lib` where $gstr and uid".$layer."='$userid' and $whi and bs=1 and $xstr and $zstr2";
		}else{
			if($v1==$userid){
		        $join = " from `$tb_lib` where userid='$v1' and $gstr and $whi and bs=1 and $xstr and $zstr";
		        $join2 = " from `$tb_lib` where userid='$v1' and $gstr and $whi and bs=1 and $xstr and $zstr2";
			}else{
		        $join = " from `$tb_lib` where userid='$v1' and uid".$layer."='$userid' and $gstr and $whi and bs=1 and $xstr and $zstr";
		        $join2 = " from `$tb_lib` where userid='$v1' and uid".$layer."='$userid' and $gstr and $whi and bs=1 and $xstr and $zstr2";
			}
		}
        ...
        $total=[];
        $zcstr = 'zc'.$layer;
        $msql->query("select sum(je),sum(je*points/100),count(id),sum($zcstr*je/100),sum($zcstr*je*points/(100*100)) $join2");
        $msql->next_record();
        $total['je'] = $msql->f(0);
        ...
        $fsql->query("select sum(je*peilv1),sum(prize),sum(je*peilv1*$zcstr/100) $join2 and z=1");
        $fsql->next_record();
        $total['zhong'] = pr1($fsql->f(0))-pr1($fsql->f(1));
        $total['bjzhong'] = pr1($fsql->f(2));
        $fsql->query("select sum(je*peilv2),sum(je*peilv2*$zcstr/100) $join2 and z=3");
        $fsql->next_record();
	...
	case "agentfl":
		...
		while ($fsql->next_record()) {
			...
			if ($jsstatus == 1) {
				...
				$bao[$i]['sendzhong'] = pr1($msql->f(3))-pr1($msql->f('sprize'));
   ...
   case "agentnew":
        ...
        $fsql->query("insert into `x_down` set  gid='$gid',userid='{$userid}',downtype='bao-----" . $_SESSION['hides'] . "',time=NOW(),jkuser='{$uid}',qishu='{$start}/{$end}'");
...
if ($jsstatus == 1) {
	...
	case "agent":
		...
		$end = rdates($_POST['end']);
		$uid = $_POST['uid'];
		if (!checkfid($uid) & $uid != $userid) exit;
		$game = $_POST['game'];
		$game = explode('|', $game);
		array_pop($game);
		$gstr = '(' . implode(',', $game) . ')';
		...
		for ($j = 0;$j < $cg;$j++) {
			...
			for ($i = 0;$i < $cb;$i++) {
				...
				if ($bao[$i]['ifagent'] == '0') {
					...
					$msql->query("select sum(peilv1*je) $join and  userid='" . $bao[$i]['userid'] . "'  and z=1 $yq $jstr");
					$msql->next_record();
					$gbao[$j][$i]['zhong'] = pr2($msql->f(0));
					$msql->query("select sum(peilv2*je) $join and  userid='" . $bao[$i]['userid'] . "'  and z=3  $yq $jstr");
					$msql->next_record();
					$gbao[$j][$i]['zhong']+= pr2($msql->f(0));
					if ($game[$j] == 100) {
						$sql = "select sum(prize) $join and userid='" . $bao[$i]['userid'] . "' and  z=5  $yq $jstr";
						$msql->query($sql);
						$msql->next_record();
						$gbao[$j][$i]['zhong']+= pr2($msql->f(0));
					}
				...
				$sql.= "  $join  and ($uidstrdown='" . $bao[$i]['userid'] . "' or userid='" . $bao[$i]['userid'] . "')  and z=3 ";
				$msql->query($sql);
				$msql->next_record();
				$gbao[$j][$i]['sendzhong']+= pr2($msql->f(0));
				if ($game[$j] == 100) {
				...
				$zbao[$i]['sendyk']+= $gbao[$j][$i]['sendyk'];
				$zbao[$i]['fly'] = 0;
				if (abs($zbao[$i]['meyk']) < 1) $zbao[$i]['meyk'] = 0;
				if (abs($zbao[$i]['sendyk']) < 1) $zbao[$i]['sendyk'] = 0;
				if (abs($zbao[$i]['yk']) < 1) $zbao[$i]['yk'] = 0;
			}
			...
			$gbao[$j][$i]['ifagent'] = 0;
			$gbao[$j][$i]['username'] = transu($uid) . "[补货]";
			$join = " from `$tb_lib`  where  gid='" . $game[$j] . "'   $yq2 $jstr";
			$msql->query("select sum(je),count(id),sum(je*points/100) $join and  userid='$uid' and z!=2 and z!=7 $yq2 $jstr");
			$msql->next_record();
			$gbao[$j][$i]['zs'] = pr0($msql->f(1));
			$gbao[$j][$i]['mezc'] = pr2($msql->f(0));
			$gbao[$j][$i]['meshui'] = pr2($msql->f(2));
			$msql->query("select sum(peilv1*je) $join and userid='$uid' and z=1 $yq2 $jstr");
			$msql->next_record();
			$gbao[$j][$i]['mezhong'] = pr2($msql->f(0));
			$msql->query("select sum(peilv2*je) $join and  userid='$uid' and z=3 $yq2 $jstr");
			$msql->next_record();
			$gbao[$j][$i]['mezhong']+= pr2($msql->f(0));
			$gbao[$j][$i]['meyk'] = pr2($gbao[$j][$i]['meshui'] + $gbao[$j][$i]['mezhong'] - $gbao[$j][$i]['mezc']);
			$gbao[$j][$i]['sendje'] = 0;
		...
		$end = microtime();
		sort($bao);
		$bao = array("gbao" => $gbao, "zbao" => $zbao, "plc" => $plc);
		echo json_encode($bao);
		unset($bao);
		unset($zbao);
		unset($gbao);
	...
	case "baouser":
		...
		$rcount = pr0($msql->f(0));
		$pcount = $rcount % $psize == 0 ? $rcount / $psize : ($rcount - ($rcount % $psize)) / $psize + 1;
		if (!is_numeric($page) | $page < 1 | $page > $pcount) $page = 1;
		if (!is_numeric($psize)) $psize = 100;
