...
try {
    ...
    // 2. 检查菜单是否存在
    $stmt = $pdo->query("SELECT COUNT(*) as count FROM la_system_menu WHERE id BETWEEN 179 AND 194");
    $menuCount = $stmt->fetch()['count'];

    echo "🔍 检查菜单数据: 找到 {$menuCount} 个盘口管理菜单\n";

    if ($menuCount == 0) {
        die("❌ 错误: 菜单数据不存在!请先执行 plate_menu_data.sql\n");
    }
